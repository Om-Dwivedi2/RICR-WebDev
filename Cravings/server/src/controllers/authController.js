...
import { getOtpToken } from "../utils/authToken.js";
...
export const UserGenOTP = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "OTP send kar diya hai registered gmail par" });
  } catch (error) {
    next(error);
  }
};

//---------------------- code me galti ho sakti hai ---------------------------------------//
...
export const UserVerifyOtp = async (req, res, next) => {
  try {
    ...
    if (!existingUserOTP) {
      const error = new Error("OTP Match Error, Plese Retry");
      error.statusCode = 401;
      return next(error);
    }
